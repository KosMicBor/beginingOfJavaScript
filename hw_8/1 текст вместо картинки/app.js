...
function showMoreText(card) {
    ...
    card.productName.insertAdjacentHTML('afterend', descGenerator());
...
function descGenerator() {
    let randomText = '', descElem = '', textForRandom = ' Lorem, ipsum dolor sit amet consectetur adipisicing elit. Consequatur eius, impedit magni fugiat necessitatibus quam sit, maxime aut tempora repudiandae distinctio laudantium accusantium asperiores! Libero molestiae, totam dolorem blanditiis nam, illo itaque esse nostrum dignissimos quia cumque. Nesciunt voluptate repellendus tenetur deserunt iste necessitatibus dolorem nam maxime, iusto consequatur explicabo odio, ad esse illum ullam voluptas quidem sed quam, quae alias quaerat officiis earum eligendi exercitationem. Eligendi minima molestiae officia ab. Natus sit expedita ab harum consectetur nulla laborum fuga nesciunt consequuntur vitae, porro iusto ea. Sapiente maxime excepturi ea illo ipsa numquam rem dignissimos facilis est, delectus incidunt culpa.';

    let textRandomArr = textForRandom.split(' ');

    for (let i = 0; i < 20; i++) {
        randomText += `${textRandomArr[Math.round(Math.random() * (textRandomArr.length - 1))]} `;
    }

    return descElem = `<div class = "desc">${randomText}</div>`;
